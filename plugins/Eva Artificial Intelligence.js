...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Eva') && eva_functionality !== 'true') {        
        ...
        try {
            if (ldet[0][0] !== 'english') {
                ceviri = await translate(finm, {from: 'auto', to: 'en'});
                trmsg = ceviri.text
            }
            else { trmsg = finm }
        } catch { 
            ceviri = await translate(finm, {
                from: 'auto', 
                to: 'en'
            });            
            trmsg = ceviri.text           
        ...
        var fins = ''                           
        if (conf.LANG !== 'EN') {
            ceviri = await translate(payload_client.result, {from: 'auto', to: conf.LANG.toLowerCase()});           
            fins = ceviri.text            
        } else { 
    ...
    try {
        ...
        convertToWav(file)
            .on('end', async () => {
                ...
                var ssc = ''
                ceviri = await translate(recognizedText, {from: 'auto', to: 'en' });               
                ssc = ceviri.text               
                var unique_ident = ''
                if (conf.WORKTYPE == 'private') {
                    unique_ident = message.client.user.jid.split('@')[0]
